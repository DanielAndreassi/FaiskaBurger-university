...
public class CategorialDAL implements IDAL<Categoria> {


    @Override
    public boolean gravar(Categoria entidade) {
        String sql = "INSERT INTO categoria (cat_nome) VALUES ('#1')";
        sql = sql.replace("#1",entidade.getNomeCategoria());
    ...
    public boolean alterar(Categoria entidade) {
        ...
        sql = sql.replace("#1",entidade.getNomeCategoria());
        sql = sql.replace("#2",""+entidade.getIdCategoria());
    ...
    public boolean apagar(Categoria entidade) {
        return SingletonDB.getConexao().manipular("DELETE FROM categoria WHERE cat_id ="+entidade.getIdCategoria());
    }
    ...
    public List<Categoria> get(String filtro) {
        ...
        try {
            while (resultSet.next()) {
                Categoria categoria = new Categoria(resultSet.getInt("cat_id"), resultSet.getString("cat_nome"));
